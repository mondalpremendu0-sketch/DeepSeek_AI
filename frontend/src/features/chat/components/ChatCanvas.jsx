...
export default function ChatCanvas({ messages, liveThinking, liveAnswer }) {
            ...
            >
                <span className="text-[10px] uppercase font-mono font-bold tracking-widest text-slate-600 px-1">
                    {isUser
                        ? "// Operator Command"
                        : isLive
                          ? "// Compiling Live Stream"
                          : "// Model Optimized Output"}
                </span>

                <div
                    className={`max-w-[90%] text-xs leading-relaxed p-4 rounded-xl border ${
                        isUser
                ...
                >
                    ...
                    {!isUser && reasoningContent && (
                        <div className="mb-4 bg-slate-950/80 border-l-2 border-slate-700 rounded-r-lg p-3 font-mono text-[11px] text-slate-400 space-y-1.5 select-text">
                            ...
                            <div className="whitespace-pre-wrap leading-normal opacity-85">
                                {reasoningContent}
                            </div>
                        </div>
                    )}
                    ...
                    {/* FIXED: Swapped global 'prose' for direct utility-driven styling injection points 
              This prevents Tailwind's layout engine from accidentally capturing or blocking vertical scrolling touch gestures.
          */}
                    <div
                        className={`w-full text-xs break-words space-y-2 select-text ${isUser ? "text-white font-medium" : "text-slate-300"}`}
                    >
                        {isUser ? (
                            ...
                            <ReactMarkdown
                                components={{
                                    ...
                                    ul: ({ children }) => (
                                        <ul className="list-disc pl-5 my-2 space-y-1 text-slate-300 block">
                                            {children}
                                        </ul>
                                    ),
                                    ol: ({ children }) => (
                                        <ol className="list-decimal pl-5 my-2 space-y-1 text-slate-300 block">
                                            {children}
                                        </ol>
                                    ),
                                    li: ({ children }) => (
                                    ...
                                    h1: ({ children }) => (
                                        <h1 className="text-xs font-bold text-slate-100 uppercase mt-3 mb-1 font-mono border-b border-slate-800/60 pb-1">
                                            {children}
                                        </h1>
                                    ),
                                    h2: ({ children }) => (
                                        <h2 className="text-xs font-bold text-slate-200 mt-2 mb-1 font-mono">
                                            {children}
                                        </h2>
                                    ),
    ...
    return (
        // FIXED: Enforced clean, standard CSS inline scrolling parameters ('overflow-y: scroll')
        // We added a native touch action utility ('touch-pan-y') to force the mobile engine to scroll the view.
        <div
            ref={containerRef}
            /* CRITICAL CLASSES: flex-1 (takes rest of screen), min-h-0 (allows shrinking), overflow-y-auto (ENABLES SCROLLING) */
            className="w-full flex-1 min-h-0 overflow-y-auto p-4 md:p-6 bg-slate-950 scroll-smooth"
            style={{ touchAction: "pan-y" }} // Unlocks smooth phone finger dragging
        >
            <div className="max-w-2xl mx-auto flex flex-col space-y-6 pb-24">
                ...
                <div ref={scrollElementRef} className="h-4 shrink-0" />
            </div>
        </div>
    );
}
